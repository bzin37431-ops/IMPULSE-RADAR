...
export function discoveryProviders(mode: 'PRINCIPAL'|'ALTERNATIVA'|'AMPLIADA'): DiscoveryProvider[] {
  if (env.DISCOVERY_TEST_MODE) return mode === 'AMPLIADA' ? [new MockDiscoveryProvider(), new MockDiscoveryProvider(), new MockDiscoveryProvider()] : [new MockDiscoveryProvider()];
