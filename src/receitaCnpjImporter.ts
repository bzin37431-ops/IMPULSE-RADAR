...
export async function importReceitaSource(options: ImportOptions) {
  ...
  for (const entry of entries.filter((item) => /munic/i.test(item.name))) for await (const row of rows(entry.stream)) { const name = row[3] || row[2] || row[1]; if (name) { municipalities.set(row[0], name); municipalities.set(row[1], name); } }
