...
export const normalizeCnpj = (value?: string) => (value || '').normalize('NFD').replace(/[.\/-]/g, '').trim().toUpperCase();
...
export class ReceitaCnpjIndexService {
  async importRecords(records: ReceitaCnpjRecord[]) {
    for (let offset = 0; offset < records.length; offset += 500) {
      ...
      await prisma.$transaction(batch.map((record) => prisma.cnpjEstablishmentIndex.upsert({
        where: { cnpj: normalizeCnpj(record.cnpj) },
  ...
  private data(record: ReceitaCnpjRecord) { return { cnpj: normalizeCnpj(record.cnpj), businessName: record.businessName, normalizedBusinessName: normalize(record.businessName), tradeName: record.tradeName || null, normalizedTradeName: record.tradeName ? normalize(record.tradeName) : null, status: normalizeStatus(record.status), cnae: record.cnae || null, street: record.street || null, number: record.number || null, district: record.district || null, city: record.city, state: record.state, postalCode: record.postalCode || null, phone1: record.phone1 || null, phone2: record.phone2 || null, email: record.email || null }; }
}
