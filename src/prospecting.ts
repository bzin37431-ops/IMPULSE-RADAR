...
export function deduplicateBusinesses(items: ReturnType<typeof normalizeBusiness>[]) {
  ...
  for (const item of items) { const ids = Object.values(item.externalIds || {}).filter(Boolean); const existing = result.find((candidate) => ids.some((id) => Object.values(candidate.externalIds || {}).includes(id)) || (item.normalizedPhone && candidate.normalizedPhone === item.normalizedPhone) || (item.address && candidate.address && normalizeName(item.address) === normalizeName(candidate.address)) || (item.domain && candidate.domain === item.domain) || candidate.normalizedName === item.normalizedName && candidate.city.toLowerCase() === item.city.toLowerCase() || item.latitude && candidate.latitude && Math.abs(candidate.latitude - item.latitude) < .0005 && Math.abs((candidate.longitude || 0) - (item.longitude || 0)) < .0005); if (!existing) result.push(item); else { existing.phone ||= item.phone; existing.website ||= item.website; existing.email ||= item.email; existing.instagram ||= item.instagram; existing.externalIds = { ...existing.externalIds, ...item.externalIds }; existing.rating = Math.max(existing.rating || 0, item.rating || 0); } }
  return result;
}
