...
test('prospecting inicia busca mock, filtra e salva oportunidade como lead', async () => { const store = new MemoryStore(true); const app = buildApp({ store }); const created = await app.inject({ method:'POST', url:'/api/prospecting/searches', payload:{ state:'São Paulo', city:'São José dos Campos', niche:'Clínicas de estética', sourceMode:'AMPLIADA', digitalStatus:'NO_WEBSITE_FOUND', minScore:20, quantity:5, coverageMode:'INTELLIGENT' } }); assert.equal(created.statusCode, 200); const id=created.json().data.id; await new Promise((resolve)=>setTimeout(resolve,50)); const result=await app.inject({ method:'GET', url:`/api/prospecting/searches/${id}` }); assert.equal(result.statusCode,200); assert.equal(result.json().search.status,'COMPLETED'); assert.ok(result.json().data.length > 0); const saved=await app.inject({method:'POST',url:`/api/prospecting/businesses/${result.json().data[0].id}/save-lead`}); assert.equal(saved.statusCode,200); assert.equal(store.leads.size,1); await app.close(); });
