...
test('catálogo de nichos tem 500 itens e está ordenado', async () => { const { BUSINESS_NICHES } = await import('../src/data/businessNiches.js'); assert.ok(BUSINESS_NICHES.length >= 500); assert.deepEqual(BUSINESS_NICHES, BUSINESS_NICHES.slice().sort((a,b)=>a.localeCompare(b,'pt-BR'))); });
